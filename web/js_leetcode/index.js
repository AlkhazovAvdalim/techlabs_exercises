...
function filter(arr, filter_out) {
    let filtered = [];
    filtered = (arr.filter(n=> !filter_out.includes(n)))
    return filtered
};
...
function sortTriplets(triplets) {
    ...
    let first_two= triplets.slice(0,2);
    let last_entry = parseInt(triplets.slice(2,3));
    let highest = Math.max(...triplets);
    let max_half =  Math.max(...first_two);
    ...
    else if (last_entry!=lowest && last_entry!=highest){
      middle = last_entry
    }
    else {
        middle = Math.min(...first_two)
...
let triplets = [3,1,2]
console.log(sortTriplets(triplets))
