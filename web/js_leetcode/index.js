...
function filter(arr, filter_out) {
  let filtered = [];
  if (arr.length > filter_out.length){
    for (var i =0; i<filter_out.length;i++){
      if (arr.includes(filter_out[i])){
      }
      else{
        filtered.push(filter_out[i]);
...
let filtered = filter(arr,filter_out)
...
swapped = swapObjectKeyValue(obj)
...
let merged = mergeObject(obj1,obj2);
// console.log(merged)
/**
...
function sortTriplets(triplets) {
  ...
  let lowest = Math.min(...triplets)
  let highest = Math.max(...triplets)
  let middle = parseInt(triplets.filter(middle => middle!=lowest && middle!=highest))
  let sorted = [lowest,middle, highest]
  return sorted
}

sortTriplets(triplets2)
